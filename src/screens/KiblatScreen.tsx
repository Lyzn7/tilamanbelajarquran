import { useSettings } from "@/store/SettingsProvider";
import { darkColors, lightColors } from "@/theme";
import { getQiblaBearing } from "@/utils/qibla";
import { Ionicons } from "@expo/vector-icons";
import * as Location from "expo-location";
import React, { useEffect, useMemo, useRef, useState } from "react";
import {
  ActivityIndicator,
  Animated,
  Dimensions,
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  View,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";

const { width } = Dimensions.get("window");
const COMPASS_SIZE = Math.min(width * 0.7, 300);

const normalizeAngle = (angle: number) => (angle + 360) % 360;
type MagnetometerModule = {
  isAvailableAsync: () => Promise<boolean>;
  setUpdateInterval: (intervalMs: number) => void;
  addListener: (listener: (data: { x: number; y: number; z: number }) => void) => {
    remove: () => void;
  };
};

const KiblatScreen: React.FC = () => {
  const { isDark } = useSettings();
  const colors = isDark ? darkColors : lightColors;

  const [locationError, setLocationError] = useState<string | null>(null);
  const [qiblaBearing, setQiblaBearing] = useState<number | null>(null);
  const [heading, setHeading] = useState(0);
  const [isReady, setIsReady] = useState(false);
  const [offset, setOffset] = useState(0);
  const holdIntervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  useEffect(() => {
    let sub: { remove: () => void } | undefined;
    let mounted = true;

    const setup = async () => {
      try {
        const { status } = await Location.requestForegroundPermissionsAsync();
        if (status !== "granted") {
          if (mounted) {
            setLocationError(
              "Izin lokasi ditolak. Kompas membutuhkan lokasi GPS Anda untuk menentukan arah Kiblat yang akurat.",
            );
          }
          return;
        }

        const location = await Location.getCurrentPositionAsync({
          accuracy: Location.Accuracy.Balanced,
        });
        if (!mounted) return;

        setQiblaBearing(
          getQiblaBearing(location.coords.latitude, location.coords.longitude),
        );

        try {
          sub = await Location.watchHeadingAsync((data) => {
            const nextHeading = data.trueHeading >= 0 ? data.trueHeading : data.magHeading;
            setHeading(normalizeAngle(nextHeading));
          });
          if (mounted) setIsReady(true);
          return;
        } catch {
          // Fall back to raw magnetometer only when OS heading is unavailable.
        }

        let magnetometer: MagnetometerModule;
        try {
          magnetometer = require("expo-sensors").Magnetometer as MagnetometerModule;
        } catch {
          setLocationError(
            "Sensor kompas belum tersedia di aplikasi ini. Rebuild aplikasi setelah memasang expo-sensors.",
          );
          return;
        }

        const isAvailable = await magnetometer.isAvailableAsync();
        if (!isAvailable) {
          if (mounted) setLocationError("Sensor kompas tidak tersedia di perangkat ini.");
          return;
        }
        magnetometer.setUpdateInterval(80);
        sub = magnetometer.addListener((data) => {
          const rawAngle = Math.atan2(data.y, data.x) * (180 / Math.PI);
          setHeading(normalizeAngle(90 - rawAngle));
        });

        if (mounted) setIsReady(true);
      } catch (err) {
        if (mounted) setLocationError("Gagal mengambil data lokasi atau sensor kompas.");
      }
    };

    setup();

    return () => {
      mounted = false;
      sub?.remove();
      if (holdIntervalRef.current) clearInterval(holdIntervalRef.current);
    };
  }, []);

  const adjustedHeading = normalizeAngle(heading + offset);
  const compassRotation = `${360 - adjustedHeading}deg`;
  const qiblaPointerRotation = `${qiblaBearing ?? 0}deg`;

  const isFacingQibla = useMemo(() => {
    if (qiblaBearing === null) return false;
    const diff = Math.abs(adjustedHeading - qiblaBearing);
    return diff <= 5 || diff >= 355;
  }, [adjustedHeading, qiblaBearing]);

  const changeOffset = (delta: number) =>
    setOffset((prev) => Math.round(normalizeAngle(prev + delta)));

  const stopHoldOffset = () => {
    if (holdIntervalRef.current) {
      clearInterval(holdIntervalRef.current);
      holdIntervalRef.current = null;
    }
  };

  const startHoldOffset = (delta: number) => {
    stopHoldOffset();
    changeOffset(delta);
    holdIntervalRef.current = setInterval(() => changeOffset(delta), 120);
  };

  if (locationError) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.centerBox}>
          <Ionicons name="warning" size={48} color={colors.accent} style={styles.centerIcon} />
          <Text style={[styles.errorText, { color: colors.text }]}>{locationError}</Text>
        </View>
      </SafeAreaView>
    );
  }

  if (!isReady || qiblaBearing === null) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.centerBox}>
          <ActivityIndicator size="large" color={colors.primary} />
          <Text style={[styles.loadingText, { color: colors.muted }]}>
            Menyiapkan kompas dan mencari lokasi Anda...
          </Text>
        </View>
      </SafeAreaView>
    );
  }

  return (
    <SafeAreaView
      style={[styles.container, { backgroundColor: colors.background }]}
      edges={["bottom", "left", "right"]}
    >
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: colors.text }]}>Kompas Kiblat</Text>
          <Text style={[styles.subtitle, { color: colors.muted }]}>
            Putar perangkat hingga penunjuk atas sejajar dengan arah Ka'bah.
          </Text>
        </View>

        <View style={[styles.disclaimer, { backgroundColor: isDark ? "#2A2418" : "#FFF8E1" }]}>
          <Ionicons name="warning-outline" size={16} color="#F59E0B" style={styles.warningIcon} />
          <View style={styles.flex}>
            <Text style={[styles.disclaimerTitle, { color: isDark ? "#FACC15" : "#B45309" }]}>
              Perhatian akurasi sensor
            </Text>
            <Text style={[styles.disclaimerText, { color: isDark ? "#FDE68A" : "#78350F" }]}>
              Sensor kompas HP bisa terpengaruh casing magnet, perangkat elektronik, atau posisi
              ruangan. Kalibrasi dulu, sesuaikan huruf T dengan arah Timur di dunia nyata,
              lalu gunakan koreksi sensor jika arah terlihat miring.
            </Text>
          </View>
        </View>

        <View style={styles.compassWrapper}>
          <View
            style={[
              styles.outerRing,
              {
                borderColor: isFacingQibla ? colors.accent : colors.border,
                backgroundColor: isFacingQibla ? colors.accent + "1A" : colors.card,
              },
            ]}
          >
            <Animated.View style={[styles.compassBase, { transform: [{ rotate: compassRotation }] }]}>
              <Text style={[styles.cardinal, styles.north, { color: colors.primary }]}>U</Text>
              <Text style={[styles.cardinal, styles.east, { color: colors.text }]}>T</Text>
              <Text style={[styles.cardinal, styles.south, { color: colors.text }]}>S</Text>
              <Text style={[styles.cardinal, styles.west, { color: colors.text }]}>B</Text>

              <View
                style={[
                  styles.qiblaIndicatorContainer,
                  { transform: [{ rotate: qiblaPointerRotation }] },
                ]}
              >
                <Ionicons name="caret-up" size={32} color={colors.accent} style={styles.qiblaCaret} />
                <View style={[styles.qiblaLine, { backgroundColor: colors.accent }]} />
              </View>

              <View style={[styles.centerIconContainer, { backgroundColor: colors.background + "E6" }]}>
                <Text style={[styles.centerText, { color: colors.text }]}>Ka'bah</Text>
              </View>
            </Animated.View>

            <View style={styles.devicePointer}>
              <Ionicons
                name="chevron-up"
                size={48}
                color={isFacingQibla ? colors.accent : colors.primary}
              />
            </View>
          </View>

          <View style={styles.infoRow}>
            <View style={styles.infoBox}>
              <Text style={[styles.infoLabel, { color: colors.muted }]}>Sudut Kiblat</Text>
              <Text style={[styles.infoValue, { color: colors.text }]}>
                {Math.round(qiblaBearing)}°
              </Text>
            </View>
            <View style={styles.infoBox}>
              <Text style={[styles.infoLabel, { color: colors.muted }]}>Arah HP</Text>
              <Text style={[styles.infoValue, { color: isFacingQibla ? colors.accent : colors.text }]}>
                {Math.round(adjustedHeading)}°
              </Text>
            </View>
          </View>
        </View>

        <View style={[styles.offsetCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.offsetTitle, { color: colors.text }]}>Koreksi Sensor</Text>
          <Text style={[styles.offsetSub, { color: colors.muted }]}>
            Geser kompas jika arah tidak sesuai kenyataan.
          </Text>
          <View style={styles.offsetRow}>
            <Pressable
              style={[styles.offsetStepBtn, { borderColor: colors.border, backgroundColor: colors.badge }]}
              onPress={() => changeOffset(-1)}
              onPressIn={() => startHoldOffset(-1)}
              onPressOut={stopHoldOffset}
            >
              <Text style={[styles.offsetStepText, { color: colors.text }]}>-</Text>
            </Pressable>
            <View style={[styles.offsetCurrent, { borderColor: colors.primary }]}>
              <Text style={[styles.offsetCurrentText, { color: colors.primary }]}>
                +{Math.round(offset)}°
              </Text>
            </View>
            <Pressable
              style={[styles.offsetStepBtn, { borderColor: colors.border, backgroundColor: colors.badge }]}
              onPress={() => changeOffset(1)}
              onPressIn={() => startHoldOffset(1)}
              onPressOut={stopHoldOffset}
            >
              <Text style={[styles.offsetStepText, { color: colors.text }]}>+</Text>
            </Pressable>
          </View>
          {offset !== 0 ? (
            <Pressable onPress={() => setOffset(0)} style={styles.resetBtn}>
              <Ionicons name="refresh" size={14} color={colors.muted} />
              <Text style={[styles.resetText, { color: colors.muted }]}>Reset koreksi</Text>
            </Pressable>
          ) : null}
        </View>

        {isFacingQibla ? (
          <View style={[styles.successBanner, { backgroundColor: colors.accent }]}>
            <Text style={styles.successText}>Arah Kiblat sudah tepat</Text>
          </View>
        ) : null}
      </ScrollView>
    </SafeAreaView>
  );
};

const styles = StyleSheet.create({
  container: { flex: 1 },
  flex: { flex: 1 },
  scroll: { flexGrow: 1, paddingBottom: 24 },
  centerBox: { flex: 1, justifyContent: "center", alignItems: "center", padding: 24 },
  centerIcon: { marginBottom: 16 },
  errorText: { textAlign: "center", fontSize: 16, lineHeight: 24 },
  loadingText: { marginTop: 16, fontSize: 14 },
  header: { padding: 20, paddingTop: 16, alignItems: "center" },
  title: { fontSize: 24, fontWeight: "800", marginBottom: 6 },
  subtitle: { fontSize: 13, textAlign: "center", lineHeight: 20 },
  disclaimer: {
    marginHorizontal: 16,
    marginBottom: 12,
    borderRadius: 12,
    padding: 12,
    flexDirection: "row",
    gap: 8,
  },
  warningIcon: { marginTop: 1 },
  disclaimerTitle: { fontSize: 13, fontWeight: "800", marginBottom: 4 },
  disclaimerText: { fontSize: 12, lineHeight: 18 },
  compassWrapper: { alignItems: "center", paddingBottom: 8 },
  outerRing: {
    width: COMPASS_SIZE,
    height: COMPASS_SIZE,
    borderRadius: COMPASS_SIZE / 2,
    borderWidth: 8,
    alignItems: "center",
    justifyContent: "center",
    shadowOpacity: 0.1,
    shadowRadius: 20,
    elevation: 8,
  },
  compassBase: {
    width: COMPASS_SIZE - 20,
    height: COMPASS_SIZE - 20,
    borderRadius: (COMPASS_SIZE - 20) / 2,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
  },
  cardinal: { position: "absolute", fontSize: 24, fontWeight: "900" },
  north: { top: 10 },
  south: { bottom: 10 },
  east: { right: 15 },
  west: { left: 15 },
  qiblaIndicatorContainer: {
    position: "absolute",
    width: "100%",
    height: "100%",
    alignItems: "center",
    justifyContent: "flex-start",
  },
  qiblaCaret: { marginTop: -15, zIndex: 11 },
  qiblaLine: { width: 4, height: COMPASS_SIZE / 2 - 35, borderRadius: 2, marginTop: -5 },
  centerIconContainer: {
    position: "absolute",
    justifyContent: "center",
    alignItems: "center",
    width: 64,
    height: 64,
    borderRadius: 32,
    zIndex: 10,
  },
  centerText: { fontSize: 12, fontWeight: "900" },
  devicePointer: { position: "absolute", top: -35, alignItems: "center" },
  infoRow: { flexDirection: "row", gap: 32, marginTop: 16 },
  infoBox: { alignItems: "center" },
  infoLabel: { fontSize: 12, fontWeight: "700", textTransform: "uppercase", letterSpacing: 1 },
  infoValue: { fontSize: 26, fontWeight: "900", marginTop: 2 },
  offsetCard: {
    marginHorizontal: 16,
    marginTop: 12,
    borderRadius: 14,
    borderWidth: 1,
    padding: 14,
  },
  offsetTitle: { fontSize: 15, fontWeight: "800", marginBottom: 2 },
  offsetSub: { fontSize: 12, marginBottom: 10 },
  offsetRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
    flexWrap: "wrap",
    justifyContent: "center",
  },
  offsetStepBtn: {
    width: 48,
    height: 40,
    borderRadius: 10,
    borderWidth: 1,
    alignItems: "center",
    justifyContent: "center",
  },
  offsetStepText: { fontSize: 22, fontWeight: "900" },
  offsetCurrent: {
    paddingHorizontal: 10,
    height: 40,
    borderRadius: 10,
    borderWidth: 2,
    minWidth: 92,
    alignItems: "center",
    justifyContent: "center",
  },
  offsetCurrentText: { fontSize: 13, fontWeight: "900" },
  resetBtn: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 8, alignSelf: "center" },
  resetText: { fontSize: 12 },
  successBanner: {
    alignSelf: "center",
    marginTop: 12,
    paddingHorizontal: 24,
    paddingVertical: 12,
    borderRadius: 30,
    elevation: 4,
  },
  successText: { color: "white", fontWeight: "800", fontSize: 15 },
});

export default KiblatScreen;
